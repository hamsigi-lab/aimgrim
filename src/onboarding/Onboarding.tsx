...
type Step = 'welcome' | 'parent' | 'child'

export function Onboarding() {
  const { setMe, enterDemo } = useAuth()
  const [step, setStep] = useState<Step>('welcome')

  if (step === 'parent') return <ParentAuth onBack={() => setStep('welcome')} onDone={setMe} />
    ...
    <div className="onb">
      ...
      <div className="stack-btns">
        ...
        <div style={{ textAlign: 'center' }}>
          <button type="button" className="linkbtn" onClick={enterDemo}>먼저 둘러보기 (체험) →</button>
        </div>
      </div>
    </div>
  )
