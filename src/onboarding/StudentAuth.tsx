...
import { studentSignup, ApiError, type Me } from '../auth/api'

const CURRENT_YEAR = new Date().getFullYear()

/** 학생 혼자(자기주도) 가입 — 만 14세 이상. 14세 미만이면 부모와 함께로 안내. */
export function StudentAuth({ onBack, onDone, onGoFamily }: { onBack: () => void; onDone: (me: Me) => void; onGoFamily: () => void }) {
  ...
  const canSubmit = emailOk && password.length >= 6 && name.trim().length > 0 && validYear && consent && !busy

  async function submit() {
    setErr(null); setTooYoung(false); setBusy(true)
    try {
      const me = await studentSignup({ email: email.trim(), password, name: name.trim(), birthYear: yearNum, consent })
      onDone(me)
    } catch (e) {
      ...
      <div className="form">
        ...
        {tooYoung && (
          <div className="inapp-note">
            🌱 만 14세 미만은 <b>부모님과 함께</b> 시작해요(개인정보 보호). 부모님 계정으로 함께하면 똑같이 스스로 관리할 수 있어요.
        ...
        <div className="field">
          ...
          {age != null && <span className="hint">만 {age}세{age < 14 ? ' · 혼자 가입은 만 14세 이상이에요' : ''}</span>}
        </div>
